...
#if M98095_LINUX_VERSION < KERNEL_VERSION(2,6,37)
#include <sound/soc-dapm.h>
#endif

#if M98095_LINUX_VERSION < KERNEL_VERSION(2,6,37)
// ZTE: added by wangyunan for hp_detect Begin
//static struct snd_soc_codec *max98095_codec;
struct snd_soc_codec *max98095_codec;
// ZTE: added by wangyunan for hp_detect End
#endif
struct snd_soc_codec *max98095_codec;
...
static const u8 max98095_reg_def[M98095_REG_CNT] = {
	...
	0x00, /* 45 */
#if 0	
#if M98095_SILICON_VERSION < M98095_REVC
	0x00, /* 46 */
#else
	0x03, /* 46 */
#endif
...
#if M98095_LINUX_VERSION < KERNEL_VERSION(2,6,39)
static int max98095_readable(unsigned int reg)
#else
...
#if M98095_LINUX_VERSION < KERNEL_VERSION(2,6,39)
static int max98095_volatile_register(unsigned int reg)
#else
...
//	case M98095_00F_HOST_CFG:
	case M98095_015_DEC:
        //ZTE: added for MIC gain setting, begin
 //       case M98095_05F_LVL_MIC1:
 //       case M98095_060_LVL_MIC2:
        //ZTE: added for MIC gain setting, end
		return 1;
	}
...
/* Add by wangyunan for headphone hook detect begin*/
int max98095_mic_detect(struct snd_soc_codec *codec, struct snd_soc_jack *jack,
		      int det, int shrt)
{
...
/* Add by wangyunan for headphone hook detect end*/
...
static const struct snd_kcontrol_new max98095_extmic_mux =
	SOC_DAPM_ENUM("External MIC", max98095_extmic_enum);
#endif
...
{
#if M98095_LINUX_VERSION < KERNEL_VERSION(2,6,38)
	snd_soc_dapm_new_controls(codec, max98095_dapm_widgets,
				  ARRAY_SIZE(max98095_dapm_widgets));

	snd_soc_dapm_add_routes(codec, max98095_audio_map, ARRAY_SIZE(max98095_audio_map));
#else
	...
	snd_soc_dapm_add_routes(dapm, max98095_audio_map, ARRAY_SIZE(max98095_audio_map));
#endif
...
{
#if M98095_LINUX_VERSION < KERNEL_VERSION(2,6,34)
...
static void max98095_sync_cache(struct snd_soc_codec *codec)
{
	...
	for (i = 1; i < ARRAY_SIZE(max98095->reg_cache); i++) {
		...
		if (max98095->reg_cache[i] == max98095_reg_def[i])
			continue;
...
#if M98095_LINUX_VERSION < KERNEL_VERSION(2,6,37)
struct snd_soc_dai max98095_dai[] = {
#else
...
#ifdef CONFIG_PM
#if M98095_LINUX_VERSION < KERNEL_VERSION(2,6,37)
static int max98095_suspend(struct platform_device *pdev, pm_message_t state)
{
...
static int max98095_suspend(struct snd_soc_codec *codec, pm_message_t state)
{
	...
	snd_soc_update_bits(codec,M98095_045_CFG_DSP,0x10,0x00);
	snd_soc_update_bits(codec,M98095_089_JACK_DET_AUTO,0x8C,0x00);
	snd_soc_update_bits(codec,M98095_093_BIAS_CTRL,0xF0,0x00);
	snd_soc_update_bits(codec,M98095_096_PWR_DAC_CK,0x3F,0x00);
	snd_soc_update_bits(codec,M98095_097_PWR_SYS,0x81,0x00);
	ret = gpio_get_value(178);
	printk("hp_detect ret = %d\n",ret);
	if(ret == 0){
		printk("hp_detect in \n");
		snd_soc_update_bits(codec,M98095_045_CFG_DSP,0x10,0x10);
		snd_soc_update_bits(codec,M98095_089_JACK_DET_AUTO,0x08,0x08);
		snd_soc_update_bits(codec,M98095_093_BIAS_CTRL,0xF0,0xF0);
...
#if M98095_LINUX_VERSION < KERNEL_VERSION(2,6,37)
static int max98095_resume(struct platform_device *pdev)
{
	struct snd_soc_device *socdev = platform_get_drvdata(pdev);
	struct snd_soc_codec *codec = socdev->card->codec;
	int i;
	...
	for (i = 0; i < M98095_REG_CNT; i++) {
		if (i == M98095_097_PWR_SYS)
			continue;

		if (!max98095_access[i].writable)
			continue;

		max98095_hw_write(codec, i, cache[i]);
	}
#else
static int max98095_resume(struct snd_soc_codec *codec)
{
	...
	snd_soc_write(codec, M98095_025_XCLKCFG, 0x6D);
	/* Resume */
	...
	snd_soc_update_bits(codec,M98095_045_CFG_DSP,0x10,0x10);
	snd_soc_update_bits(codec,M98095_089_JACK_DET_AUTO,0x8C,0x8C);
	snd_soc_update_bits(codec,M98095_093_BIAS_CTRL,0xF0,0xF0);
...
#if 0
//ZTE: added by hn for test, begin
#include <linux/proc_fs.h>
#define CODEC_PROC_FILE "driver/codec"
static struct proc_dir_entry *codec_proc_file;
extern struct proc_dir_entry *create_proc_entry(const char *name, mode_t mode,
					 struct proc_dir_entry *parent);
extern void remove_proc_entry(const char *name, struct proc_dir_entry *parent);
static u8 ascii_to_u8(char *buffer)
{
     u8 reg_codec = 0;
     
     if (buffer[0] >= 'A' && buffer[0] <= 'F')
     {
        reg_codec = (buffer[0] - 'A' + 10) << 4;
     }
     else if (buffer[0] >= '0' && buffer[0] <= '9')
     {
        reg_codec = (buffer[0] - '0') << 4;
     }

     if (buffer[1] >= 'A' && buffer[1] <= 'F')
     {
        reg_codec += buffer[1] - 'A' + 10 ;
     }
     else if (buffer[1] >= '0' && buffer[1] <= '9')
     {
        reg_codec += buffer[1] - '0';
     }

     return reg_codec;
}

static ssize_t codec_proc_read(struct file *filp,
				   ...
				   loff_t * offset)
{
    ...
    for(reg_codec=0;reg_codec<=M98095_REG_MAX_CACHED;reg_codec++)
    {
        reg_val = snd_soc_read(max98095_codec, reg_codec);	
	    printk("codec_proc_read  reg_add = %02x reg_val = %02x \n",reg_codec, reg_val);	
    }
				    ...
				    loff_t * off)
{
	...
	copy_from_user(messages, buff, len);
      
       reg_addr = ascii_to_u8(&messages[0]);
       reg_val = ascii_to_u8(&messages[3]);

       snd_soc_write(max98095_codec, reg_addr, reg_val);
       printk("max98095_write reg_add = %x reg_val = %x \n", reg_addr, reg_val);
...
#if M98095_LINUX_VERSION < KERNEL_VERSION(2,6,37)
static int max98095_probe(struct platform_device *pdev)
{
	struct snd_soc_device *socdev = platform_get_drvdata(pdev);
...
static int max98095_probe(struct snd_soc_codec *codec)
{
...
#endif
	printk("[snd card]  max98095_probe \n");
#if M98095_LINUX_VERSION < KERNEL_VERSION(2,6,37)
	if (!max98095_codec) {
		dev_err(&pdev->dev, "Codec device is not registered\n");
		return -EINVAL;
	}
	socdev->card->codec = max98095_codec;
	codec = max98095_codec;
	max98095 = snd_soc_codec_get_drvdata(codec);
...
#if 0
        //ZTE:added by hn 
        codec->write = max98095_hw_write;
	codec->read = max98095_hw_read;
#endif
...
#if 0
	if (ret != M98095_SILICON_VERSION) {
		dev_err(codec->dev, "Error: Codec HW/SW version mismatch\n");
		ret = -EINVAL;
		goto err_access;
	}
#endif
	...
	snd_soc_write(codec, M98095_048_MIX_DAC_LR,
		...
		M98095_DAI2M_TO_DACL|M98095_DAI3M_TO_DACL|
		M98095_DAI2M_TO_DACR|M98095_DAI3M_TO_DACR);

	//remove by wangyunan for bluetooth
	/*
	...
	*/
	...
	//snd_soc_update_bits(codec, M98095_093_BIAS_CTRL,0x0C,0x0C);
...
#if M98095_LINUX_VERSION < KERNEL_VERSION(2,6,33)
...
#if M98095_LINUX_VERSION < KERNEL_VERSION(2,6,37)
static int max98095_remove(struct platform_device *pdev)
{
	struct snd_soc_device *socdev = platform_get_drvdata(pdev);
	struct snd_soc_codec *codec = socdev->card->codec;
#else
...
#if M98095_LINUX_VERSION < KERNEL_VERSION(2,6,37)
struct snd_soc_codec_device soc_codec_dev_max98095 = {
	.probe   = max98095_probe,
	.remove  = max98095_remove,
	.suspend = max98095_suspend,
	.resume  = max98095_resume,
};
EXPORT_SYMBOL_GPL(soc_codec_dev_max98095);
#else
			     ...
			     const struct i2c_device_id *id)
{
	...
	int i;
#endif
	printk("[snd card]  max98095_i2c_probe \n");
	max98095 = kzalloc(sizeof(struct max98095_priv), GFP_KERNEL);
//printk("=== max98095_i2c_probe2	 	max98095 = %d	in \n",max98095);	
	if (max98095 == NULL)
		return -ENOMEM;
	...
	snd_soc_codec_set_drvdata(codec, max98095);
	codec->control_data = i2c; /* max98095->control_data */
	codec->dev = &i2c->dev;

	if (max98095_codec) {
		dev_err(codec->dev, "Another MAX98095 is registered\n");
		ret = -EINVAL;
		goto err;
	}
	...
	codec->num_dai = ARRAY_SIZE(max98095_dai);
	...
	codec->set_bias_level = max98095_set_bias_level;

	codec->dai = max98095_dai;

	codec->reg_cache_size = M98095_REG_CNT;
	codec->reg_cache = &max98095->reg_cache;
	codec->volatile_register = max98095_volatile_register;

	/* power on device */
	max98095_codec = codec;

	for (i = 0; i < codec->num_dai; i++)
		max98095_dai[i].dev = codec->dev;
	...
	ret = snd_soc_register_dais(&max98095_dai[0], codec->num_dai);
...
err:
	...
	if (ret < 0)
		kfree(max98095);
	printk("[snd card]  max98095_i2c_probe ok\n");
	return ret;
#endif
}
...
{
#if M98095_LINUX_VERSION < KERNEL_VERSION(2,6,37)
	struct max98095_priv *max98095 = i2c_get_clientdata(client);
	struct snd_soc_codec *codec = &max98095->codec;

	snd_soc_unregister_dais(max98095_dai, codec->num_dai);
	snd_soc_unregister_codec(codec);
	kfree(max98095);
	max98095_codec = NULL;
#else
