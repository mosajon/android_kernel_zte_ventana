...
#define MAX98095_MUSIC_SPEAKER_VOLUME 0x19  // 4dB
#define MAX98095_MUSIC_HEADSET_VOLUME 0x12  //-11dB
			  ...
			  unsigned int value)
{
	...
	u8 test_val = 0;
         ...
         if (reg == M98095_00F_HOST_CFG)
         {
              flag = value & M98095_SEG;
         }	       
	reg &= 0xff;
	data[0] = reg;
	data[1] = value & 0xff;
	...
	if( (!snd_soc_codec_volatile_register(codec, reg) &&
	    reg < codec->driver->reg_cache_size)&&( 0x00 == flag))
		reg_cache[reg] = value;
...
#if 0
static unsigned int max98095_hw_read(struct snd_soc_codec *codec, unsigned int reg)
{
...
#if 0
		max98095_hw_write(codec, M98095_064_LVL_HP_L, MAX98095_MUSIC_HEADSET_VOLUME);
		max98095_hw_write(codec, M98095_065_LVL_HP_R, MAX98095_MUSIC_HEADSET_VOLUME);
#endif
				   ...
				   struct snd_soc_dai *dai)
{
	...
	if (snd_soc_read(codec, M98095_034_DAI2_FORMAT) & M98095_DAI_MAS) {
		...
		snd_soc_write(codec, M98095_033_DAI2_CLKCFG_LO,
			ni & 0xFF);
#else
		snd_soc_write(codec, M98095_032_DAI2_CLKCFG_HI, 0x10);
		snd_soc_write(codec, M98095_033_DAI2_CLKCFG_LO, 0x1f);
#endif
	}
				 ...
				 unsigned int fmt)
{
	...
	cdata = &max98095->dai[1];
      ...
      snd_soc_write(codec, M98095_035_DAI2_CLOCK, 0x27);
       ...
       snd_soc_write(codec, M98095_060_LVL_MIC2, 0x20);
...
err:
	...
	ret = snd_soc_register_codec(&i2c->dev,
			&soc_codec_dev_max98095, &max98095_dai[0], 2);
	if (ret < 0)
		kfree(max98095);
