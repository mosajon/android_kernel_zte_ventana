...
//currently not used, confirmed with preetam
#if 0
			 ...
			 struct snd_ctl_elem_value *ucontrol)
{
	...
	if (!strcmp(kcontrol->id.name, "Multiconfig support for DAC"))
		val = aic3262->current_dac_config[mode];
	else if (!strcmp(kcontrol->id.name, "Multiconfig support for ADC"))
		val = aic3262->current_adc_config[mode];
			 ...
			 struct snd_ctl_elem_value *ucontrol)
{
	...
	struct aic3262_priv *aic3262 = snd_soc_codec_get_drvdata(codec);
	u32 val;
	int pf = aic3262->process_flow;
	struct multibyte_config *array;

	val = ucontrol->value.integer.value[0];
	if (!strcmp(kcontrol->id.name, "Multiconfig support for DAC")) {
		if (aic3262->process_flow == MINIDSP_DMODE) {
			...
			minidsp_multibyte_transfer(codec,
				array->regs,
		...
		aic3262->current_dac_config[pf] = val;

	} else {
		if (aic3262->process_flow == MINIDSP_AMODE) {
		...
		aic3262->current_adc_config[pf] = val;
	}
	return val;
}
...
printk("======in the minidsp_multiconfig function==== \n");
...
/*
 *--------------------------------------------------------------------------
 * Function : multibyte_coeff_change
 * Purpose :  Function which is invoked when user changes the configuration
 *            at run-time. Internally configures/switches both
 *            miniDSP_D and miniDSP_A Coefficient arrays.
 *---------------------------------------------------------------------------
...
static int multibyte_coeff_change(struct snd_soc_codec *codec, int bk)
{
	...
	value = dsp_read(codec, bk, reg); 
	swap_reg_pre = value;

	/* Write the Register bit updates */
	value = value | 1;
	
	dsp_write(codec, bk, reg, value); 
	...
	mdelay(5);
	swap_reg_post = dsp_read(codec, bk, reg);
	if ((swap_reg_pre == 4 && swap_reg_post == 6)
		|| (swap_reg_pre == 6 && swap_reg_post == 4))
		printk(KERN_INFO "Buffer swap success\n");
	else {
		printk(KERN_ERR
		"Buffer swap...FAILED\nswap_reg_pre=%x, swap_reg_post=%x for book = %d\n",
		swap_reg_pre, swap_reg_post, bk);
	}
