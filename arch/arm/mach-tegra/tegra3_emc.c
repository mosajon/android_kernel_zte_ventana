...
/* ZTE: Add for read DDR ID by dongjinguang on 20120215 ++ */
...
/* ZTE: Add  read DDR ID by dongjinguang on 20120215 -- */
...
/* ZTE: read LPDDR2 memory modes by dongjinguang on 20120215 ++  */
static int tegra_emc_read_mrr(unsigned long addr)
{
	...
	if (count == 0) {
		printk("%s: Failed to read memory type\n", __func__);
		BUG();
	}
	value = (1 << 30) | (addr << 16);
...
int zte_tegra_emc_get_dram_id(void)
{
	...
	printk("[dong] line %d  function %s with ddr device ID = 0x%x\r\n " , __LINE__, __func__,  vid);
       return (vid&0xFF);
}
/* ZTE: read LPDDR2 memory modes by dongjinguang on 20120215 ++  */
