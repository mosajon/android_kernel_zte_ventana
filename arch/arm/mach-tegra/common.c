...
static bool is_tegra_debug_bootmode;
static int s_zte_board_id= -1;
...
{
	char *p = info;
	if (!strncmp(p, "max98095", 8))
		audio_codec_name = audio_codec_max98095;
	else
...
static int __init zte_boot_mode_get(char *info)
{
	char *p = info;
	if (!strncmp(p, "boot_mode=1", 11))
		return 1;
	else if (!strncmp(p, "boot_mode=0", 11))
		return 0;
}
__setup("boot_mode=", zte_boot_mode_get);

static int __init tegra_debug_bootmode(char *info)
{
	if(!strcmp(info,"rtc_alarm"))
		is_tegra_debug_bootmode = true;
	else if(!strcmp(info,"on_key"))
		is_tegra_debug_bootmode = false;

	return 1;
}
__setup("androidboot.mode=", tegra_debug_bootmode);

void tegra_get_board_info(struct board_info *bi)
{
	bi->board_id = (system_serial_high >> 16) & 0xFFFF;
	s_zte_board_id = bi->board_id; // ZTE:added by pengtao
	...
	printk("board info: Id:%d SKU:%d Fab:%d Rev:%c MinRev:%d\n",
			bi->board_id,
			bi->sku, bi->fab, bi->major_revision, bi->minor_revision); // ZTE: added by pengtao
...
int zte_get_flag(ZtePowerOnMode_E e_flag_type)
{
    int temp = -1;

    if (e_flag_type < ZTE_FLAGS_MAX)
        temp =  (g_zte_poweron_mode >> e_flag_type) & 0x1;
    printk("zte_get_flag: tag %d, data %d", e_flag_type, temp );
    return temp;
}
