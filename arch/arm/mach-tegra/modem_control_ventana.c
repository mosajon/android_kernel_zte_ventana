...
static int modem_power_enable(int enable)
{
    printk(KERN_INFO "modem_power_enable %d\n", enable);
    if(enable)
        gpio_set_value(p903u_modem_power_enb, 1);
    else
        gpio_set_value(p903u_modem_power_enb, 0);

    return 0;
}

/*ZTE: added by zhanming  for P903U modem UART switch 20110809 ++*/
int modem_uart_enable(int enable)
{  
    if( 1== enable )
    {          
...
static int modem_power_key(int enable)
{
    printk(KERN_INFO "modem_power_key %d\n", enable);
    if(enable)
        gpio_set_value(p903u_modem_power_key, 1);
    else
        gpio_set_value(p903u_modem_power_key, 0);
...
{      
    ...
    if(enable)
        gpio_set_value(p903u_usb_port_ctrl, 0);
    else
        gpio_set_value(p903u_usb_port_ctrl, 1);
...
static int modem_usb3v3_enable(int enable)
{
    ...
    if(enable)
        gpio_set_value(p903u_modem_usb3v3_enb, 1);
    else
        gpio_set_value(p903u_modem_usb3v3_enb, 0);
		...
		struct device_attribute *attr, char *buf)
{
    ...
    value = gpio_get_value(P903u_modem_dead);
    if (value < 0)
        return value;
...
static int __devinit modem_control_probe(struct platform_device *pdev)
{
    ...
    ret = gpio_request(p903u_modem_power_enb, "modem_power");
    if (ret < 0) {
        printk(KERN_WARNING "failed to request gpio modem_power %d\n", ret);
    }
    gpio_direction_output(p903u_modem_power_enb, 1);
    tegra_gpio_enable(p903u_modem_power_enb);
    
    ret = gpio_request(P903u_modem_reset_en, "modem_power");
    if (ret < 0) {
        printk(KERN_WARNING "failed to request gpio modem_reset %d\n", ret);
    }
    gpio_direction_output(P903u_modem_reset_en, 1);
    tegra_gpio_enable(P903u_modem_reset_en);
    gpio_set_value(P903u_modem_reset_en, 0);
    
    ret = gpio_request(p903u_usb_port_ctrl, "usb_switch");	
    if (ret < 0) {
        printk(KERN_WARNING "failed to request gpio usb_switch %d\n", ret);
    }
    tegra_gpio_enable(p903u_usb_port_ctrl);
    printk(KERN_INFO "usb_switch_state: %d\n", usb_switch_state);
    gpio_direction_output(p903u_usb_port_ctrl, usb_switch_state);
    
    ret =  gpio_request(p903u_modem_usb3v3_enb, "modem_usb3v3");	
    if (ret < 0) {
        printk(KERN_WARNING "failed to request gpio modem_usb3v3 %d\n", ret);
    }
    tegra_gpio_enable(p903u_modem_usb3v3_enb);
    gpio_direction_output(p903u_modem_usb3v3_enb, 1);

    ret =  gpio_request(p903u_modem_power_key, "modem_powerkey");	
    if (ret < 0) {
        printk(KERN_WARNING "failed to request gpio modem_powerkey %d\n", ret);
    }
    tegra_gpio_enable(p903u_modem_power_key);
    gpio_direction_output(p903u_modem_power_key, 1);

    ret =  gpio_request(p903u_modem_download_en, "modem_download");	
    if (ret < 0) {
        printk(KERN_WARNING "failed to request gpio modem_download %d\n", ret);
    }
    tegra_gpio_enable(p903u_modem_download_en);
    gpio_direction_output(p903u_modem_download_en, 1);
...
/*ZTE:added by zhanming for modem wakeup ap  20111104  --*/
     ...
     /*ZTE:add by caixiaoguang 20111021 for modem dead++*/   
    ret =  gpio_request(P903u_modem_dead, "modem_dead");	
    if (ret < 0) {
        printk(KERN_WARNING "failed to request gpio modem_dead %d\n", ret);
    }
    tegra_gpio_enable(P903u_modem_dead);
    gpio_direction_input(P903u_modem_dead);
    /*ZTE:add by caixiaoguang 20111021 for modem dead--*/  

    ret =  gpio_request(p903u_uart_port_switch, "modem_uart_enable");	
    if (ret < 0) {
        printk(KERN_WARNING "failed to request gpio modem_uart enable %d\n", ret);
    }
    tegra_gpio_enable(p903u_uart_port_switch);
    gpio_direction_output(p903u_uart_port_switch, 1);
...
static int __devexit modem_control_remove(struct platform_device *pdev)
{
    ...
    gpio_free(p903u_modem_power_enb);
    gpio_free(p903u_usb_port_ctrl);
    gpio_free(p903u_modem_usb3v3_enb);
    gpio_free(p903u_modem_power_key);
    gpio_free(p903u_modem_download_en);
    gpio_free(p903u_uart_port_switch);
    return 0;
}
