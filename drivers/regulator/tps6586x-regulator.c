...
int modulate_intensity(unsigned char flag)
{
	int ret = 0;
	uint8_t val = 0;
	...
	if ((flag >= 1)&&(flag <=4))
	{
		...
		//printk("get 0x5b reg val:0x%x\n",val);
    	val = (val & 0xf0) + (flag*4 - 1);
		if(flag == 1)
		{
			val += 2; 
		}
    	//val = ((7 - flag) << 4) + 15;
		//printk("write to 0x5b reg val:0x%x\n",val);
		...
		//printk("after write,get 0x5b reg val:0x%x\n",val);
	}
	else
	{
...
int tps6586x_vibrate_intensity(unsigned char flag)
{
	if((!flag) || (flag >4))
	{
		printk("no intensity parameter\n");
		return -1;
	}

	modulate_intensity(flag);

}
EXPORT_SYMBOL(tps6586x_vibrate_intensity);
