...
//ZTE:add by caixiaoguang 20111129 for lowpoer interrupt
extern irqreturn_t tps6591x_low_power_irq(int irq, void *data);
...
static int __devinit tps6591x_rtc_probe(struct platform_device *pdev)
{
	...
	tps6591x_read((&pdev->dev)->parent,0x51,&reg);
	printk("in rtc-tps6591x probe: get 0x51 reg: 0x%x\n",reg);

 /*ZTE:add by caixiaoguang 20111129 for lowpoer interrupt  ++*/
    #if 1   //closed by zhanming 20111206
    err = request_threaded_irq(752, NULL, tps6591x_low_power_irq,
					IRQF_ONESHOT, "low_power_tps6591x",
					&pdev->dev);
    if (err) 
    {
        dev_err(&pdev->dev, "request IRQ:%d fail\n", 752);
 ...
 /*ZTE:add by caixiaoguang 20111129 for lowpoer interrupt  --*/
        ...
        create_set_power_off_flag_proc_file();/*ZTE:add by caixiaoguang for set a flag before power off*/
	return 0;
...
static ssize_t rtc_proc_read(struct file *filp, const char *buff, size_t len, loff_t *off)
{
    ...
    if(p_dev == NULL)
        ...
        ret = tps6591x_read_regs(p_dev,(0x10),7,reg_val);
        for(i = 0;i < 7;i++)
            {
                
                printk("rtc reg address: 0x%x,value:0x%x\n",(0x10+i),reg_val[i]);
            }

        if(reg_val[1] & 0x40)
            {
                printk("reg_val[1] = 0x%x,write 1 to clear it\n",reg_val[1]);
                data = 0x40;
                tps6591x_write_regs(p_dev,0x11,1,&data);
            }

        ret = tps6591x_read_regs(p_dev,(0x50),6,reg_val);        
        for(i = 0;i < 6;i++)
            {
