...
void SiiRegWrite ( SiiReg_t virtualAddr, uint8_t value )
{
	...
	switch(pPage->address)
		{
		case DEV_PAGE_TPI_0:
			TX_I2C_DEBUG_PRINT(("[MHL] pPage->address:%02x is 0x72!\n",pPage->address));
		...
		case DEV_PAGE_TX_L0_0:	
			...
			*/
		case DEV_PAGE_TX_L1_0:
			TX_I2C_DEBUG_PRINT(("[MHL] pPage->address:%02x is 0x7A!\n",pPage->address));
			i2c_smbus_write_byte_data(sii8334_PAGE_TX_L1, regOffset, value);
			break;
		case DEV_PAGE_TX_2_0:
			TX_I2C_DEBUG_PRINT(("[MHL] pPage->address:%02x is 0x92!\n",pPage->address));
			i2c_smbus_write_byte_data(sii8334_PAGE_TX_2, regOffset, value);
			break;
		case DEV_PAGE_TX_3_0:
			TX_I2C_DEBUG_PRINT(("[MHL] pPage->address:%02x is 0x9A!\n",pPage->address));
			i2c_smbus_write_byte_data(sii8334_PAGE_TX_3, regOffset, value);
			break;
		case DEV_PAGE_CBUS:
			TX_I2C_DEBUG_PRINT(("[MHL] pPage->address:%02x is 0xc8!\n",pPage->address));
			i2c_smbus_write_byte_data(sii8334_PAGE_CBUS, regOffset, value);
			break;
		default:
			TX_DEBUG_PRINT(("[MHL] pPage->address:%02x is wrong!\n",pPage->address));
			break;
		}
}
