...
static irqreturn_t tegra_i2c_isr(int irq, void *dev_id)
{
	...
	if (i2c_dev->cont_id == 2 && int_mask == 0 && status == 0x00000002) {
		dev_warn(i2c_dev->dev, "unexpected interrupt status 0x%02x\n",
						status);
		if (!i2c_dev->irq_disabled) {
			disable_irq_nosync(i2c_dev->irq);
			i2c_dev->irq_disabled = 1;
...
extern int __read_mostly did_panic;    //add by zhanming for reboot flag 20120305
	...
	struct i2c_msg *msg, int stop)
{
    ...
    if(did_panic)//add by zhanming for reboot flag 20120305
    {
        mdelay(10);
        ret = 1;
    }
