...
#ifdef FOR_NET_RESOLVE_AUTO_POWERON 
	ret = tps6591x_write(dev, 0x51, 0xff);
	if (ret < 0)
		return ret;
#endif
...
EXPORT_SYMBOL_GPL(tps6591x_power_off);

int tps6591x_redlight_ctrl(int argument);
int tps6591x_greenlight_ctrl(int argument);
int tps6591x_map_led2_to_gpio3_out_low(void);
int tps6591x_auto_on_usb(void);

int tps6591x_pre_power_off(void)
{
	int ret;

    ret = tps6591x_map_led2_to_gpio3_out_low();
    if(ret < 0)
    {
        printk("close red pwm led error !!!\n");
        return -1;
    }    
    ...
    if(ret < 0)
    {
        printk("close green led error !!!\n");
        return -1;
    }

    ret = tps6591x_auto_on_usb();
    if(ret < 0)
    {
        printk("set usb auto poweron error !!!\n");
        return -1;
    }   
    
	return 0;
}
EXPORT_SYMBOL_GPL(tps6591x_pre_power_off);
...
int tps6591x_redlight_ctrl(int argument)
{
    ...
    {
        printk("argumen is not correct\n");
        return -1;
    }

    dev = &tps6591x_i2c_client->dev;

    ret = tps6591x_read(dev,0x63,&read_val);
    if(ret < 0)
        return ret;

    ret = zte_get_board_id();
    if(ret < 0)
    {
        printk("failed to get borad id\n");
        return -1;
    }
    else if(ret < 2)
    {   
    ...
    tps6591x_write(dev, 0x63, read_val);     //red light

    return 0;
}
EXPORT_SYMBOL(tps6591x_redlight_ctrl);
...
int tps6591x_greenlight_ctrl(int argument)
{
    ...
    ret = tps6591x_read(dev,0x61,&read_val);
    if(ret < 0)
        return ret;
    ...
    tps6591x_write(dev, 0x61, read_val);     //red light
    if(ret < 0)
    return ret;

    return 0;
}
EXPORT_SYMBOL(tps6591x_greenlight_ctrl);

/*ZTE:add by caixiaoguang for set a flag before power off++*/
bool power_off_flag = 0;
void tps6591x_set_power_off_flag(void)
{
    power_off_flag = 1;
    printk("tps6591x app set power off flag\n");
}
EXPORT_SYMBOL(tps6591x_set_power_off_flag);
/*ZTE:add by caixiaoguang for set a flag before power off--*/

int tps6591x_set_reboot_flag(int cmd)
{
    struct device *dev = NULL;
    int ret;
    uint8_t read_val = 0;
    ...
    if(0 == power_off_flag)
    {
        ...
        {
            read_val |= 0x1;
            ret = tps6591x_write(dev, 0x70, read_val);
            if(ret < 0)
            {
                printk("x1 write 0x70 failure !!!\n");
                return ret;
            }
        }
        else if(2 == cmd)
...
EXPORT_SYMBOL(tps6591x_set_reboot_flag);


int tps6591x_map_gpio3_to_led2_blink( void )
{
	struct device *dev = NULL;
	int ret;
	uint8_t read_val = 0;
	...
	/*#1: LED_CTRL1_REG(0x6C)-->To choose LED2 peroid:8sec*/
	ret = tps6591x_read(dev,0x6c,&read_val);
	if(ret < 0)
       	return ret;

	read_val &= (~0x38);
	read_val |= (0x7 << 3); //LED2 period:8sec

	tps6591x_write(dev, 0x6c, read_val);
	if(ret < 0)
		return ret;

	/*#2: LED_CTRL2_REG(0x6D)-->To choose LED2 pulse sequence and on time*/
	ret = tps6591x_read(dev,0x6d,&read_val);
	if(ret < 0)
       	return ret;

	read_val &= (~0x2c);
	read_val |= (0x3 << 2); //LED2 on time:500ms
	read_val |= (0x0 << 5); //One pulse: ON--OFF	
	tps6591x_write(dev, 0x6d, read_val);
	if(ret < 0)
		return ret;	
	
	/*#3: GPIO3_REG(0x63)-->To select GPIO3 as LED2*/	
	ret = tps6591x_read(dev,0x63,&read_val);
	if(ret < 0)
       	return ret;

	read_val &= (~0x60);
	read_val |= (0x1 << 5); //gpio3 selection:LED2

	tps6591x_write(dev, 0x63, read_val);
	if(ret < 0)
		return ret;
    
	return 0;	
}
EXPORT_SYMBOL(tps6591x_map_gpio3_to_led2_blink);
...
int tps6591x_map_led2_to_gpio3_out_low( void )
{
	...
	/*#: GPIO3_REG(0x63)-->To select GPIO3 as gpio*/	
	ret = tps6591x_read(dev,0x63,&read_val);
	if(ret < 0)
       	return ret;

	read_val &= (~0x65);
	read_val |= (0x0 << 5); //gpio3 selection:gpio
	read_val |= (0x1 << 2); //gpio3 as output
	read_val |= (0x0 << 0); //gpio3 output low	
...
EXPORT_SYMBOL(tps6591x_map_led2_to_gpio3_out_low);
...
int tps6591x_auto_on_usb(void)
{
    ...
    ret = tps6591x_read(dev,0x64,&read_val);
    read_val &= 0xf3;         //bit 6,5 = 0--gpio    bit 3=0 disable pulldown  bit2 = 0 input  
    ret = tps6591x_write(dev, 0x64, read_val);  
    if(ret < 0)
        return ret;

    ret = tps6591x_read(dev,0x55,&read_val);  //gpio4
    read_val &= (~0x01);   //bit 0 = 0  rising detect interrupt unmask
    ret = tps6591x_write(dev,0x55,read_val);  
    if(ret < 0)
        return ret;
...
int tps6591x_set_comp2_threshold(int threshold )
{
    struct device *dev = NULL;
    uint8_t reg_val;  
	int ret;
    ...
    if(0 == threshold)
    {
        //set comp2 threshold 3.4v    
        ret = tps6591x_read(dev, 0x6B, &reg_val);    
        if (ret < 0)
            return ret;  
        reg_val &= 0xc0;
        reg_val |= 0x2f;
        tps6591x_write(dev, 0x6B, reg_val);/*3.45V*/
        if (ret < 0)
            return ret;  

        printk("tps6591x set low power int voltage = 3.45V\n");
    }
    else
    {
        //set comp2 threshold 3.3v    
        ret = tps6591x_read(dev, 0x6B, &reg_val);  
        if (ret < 0)
            return ret;  
        reg_val &= 0xC0;
        reg_val |= 0x2b;
        tps6591x_write(dev, 0x6B, reg_val);/*3.35V*/
        if (ret < 0)
            return ret;  

        printk("tps6591x set low power int voltage = 3.35V\n");
    }
    
    return ret;       
}
EXPORT_SYMBOL_GPL(tps6591x_set_comp2_threshold);

int tps6591x_clr_comp2_int_status(void)
{
    struct device *dev = NULL;
    uint8_t reg_val;  
	int ret;
    ...
    ret = tps6591x_read(dev, 0x54, &reg_val);   
    if (ret < 0)
        return ret;   
    
    reg_val |= 0x40;
    ret = tps6591x_write(dev, 0x54, reg_val);/*clear int status*/
    if (ret < 0)
        return ret;   

    return 0;
}
EXPORT_SYMBOL_GPL(tps6591x_clr_comp2_int_status);

int tps6591x_comp2_int_enable(int enable)
{
    struct device *dev = NULL;
    uint8_t reg_val;  
	int ret;
    ...
    ret = tps6591x_read(dev, 0x55, &reg_val);   
    if (ret < 0)
        return ret;  
    ...
    ret = tps6591x_write(dev, 0x55, reg_val);
    if (ret < 0)
        return ret;

    return 0;
}
EXPORT_SYMBOL_GPL(tps6591x_comp2_int_enable);
					...
					const struct i2c_device_id *id)
{
	...
	/*disable PMU's watchdog*/
	tps6591x_write(&client->dev, 0x69, 0x00);

	/*enable soc sleep*/
	ret = tps6591x_read(&client->dev, 0x62, &reg_val);
	if (ret)
            return ret;
    
	reg_val |= 0x80;
	tps6591x_write(&client->dev, 0x62, reg_val);

	ret = tps6591x_read(&client->dev, 0x43, &reg_val);
	if (ret)
            return ret;
    
	reg_val |= 0x30;
	tps6591x_write(&client->dev, 0x43, reg_val);

	ret = tps6591x_read(&client->dev, 0x60, &reg_val);
	if (ret)
            return ret;
    
	reg_val |= 0x80;
	tps6591x_write(&client->dev, 0x60, reg_val);


	tps6591x_i2c_client = client;

	//anto power_on when insert charger        
	if(zte_get_board_id() >= 2) //add judge of version
	{
            //printk("new board:enable usb power_on");
            tps6591x_auto_on_usb();
	}
