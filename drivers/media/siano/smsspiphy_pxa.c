...
#include "zi_cmmb_api.h" /*shangmingsen 20120417 */

extern Zi_INT32 cmmb_chip_type;
Zi_VOID Zi_irq_level(Zi_UINT8 iplace);
...
#ifdef ZTE_CMMB_ZM2167
static void Zi_chip_poweron(void)
{
	printk("[cmmbspi]chip_poweron \n");
	//gpio_set_value(TEGRA_GPIO_PA3, 1);
	gpio_set_value(TEGRA_GPIO_PX2, 1);
...
static void Zi_chip_powerdown(void)
{
...
Zi_VOID Zi_irq_level(Zi_UINT8 iplace)
{
	//Zi_log("Place[%d], irq level:%d\n", iplace, gpio_get_value(TEGRA_GPIO_PX1));
}
#endif
...
int smsspibus_ssp_resume(void* context) 
{
...
#ifdef ZTE_CMMB_ZM2167
	if(cmmb_chip_type == CMMB_UNKOWN_DEVICE)
	{
    	      Zi_chip_poweron();
		Zi_Chip_Test();
		Zi_chip_powerdown();
	...
	if( CMMB_ZM2167_DEVICE == cmmb_chip_type) //add by mingsen
	{
		Zi_chip_poweron();
		Zi_OpenDevice();
		ret = request_irq(TEGRA_GPIO_TO_IRQ(TEGRA_GPIO_PX1), spibus_interrupt,
			IRQ_TYPE_EDGE_FALLING, "SMSSPI", spiphy_dev); 
	}
	else
#endif
	{
