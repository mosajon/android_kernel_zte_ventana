...
/* add by mingsen */
Zi_UINT8 *Zi_dev_area = NULL;
Zi_UINT32 Zi_dev_size;
...
#ifdef ZTE_CMMB_ZM2167	
	Zi_dev_size = 5*64*1024;
	if(dev->common_buffer_size  < Zi_dev_size)
		dev->common_buffer_size = Zi_dev_size;
#endif	
...
#ifdef ZTE_CMMB_ZM2167	
	Zi_dev_area = (Zi_UINT8 *)dev->common_buffer;
#endif
	/* prepare dma buffers */
...
#ifdef ZTE_CMMB_ZM2167
extern int smschar_wakeup_spi(void *context);

static struct smscore_client_t *smscore_find_client2(
		struct smscore_device_t *coredev, int data_type, int id) {
	struct smscore_client_t *client = NULL;
	struct smscore_client_t *clienttest = NULL;
	struct list_head *next, *first;
	unsigned long flags;
	struct list_head *firstid, *nextid;
	int i=0;
	...
	for (next = first->next; (next != first) && !client;
		...
		for (nextid = firstid->next; nextid != firstid;
				nextid = nextid->next) {
		clienttest = (struct smscore_client_t *) next;
		//printk(KERN_INFO"[zm2167]smscore_find_client2(%d)  coredev=%d  data_type=%d  id=%d clienttest->context=%d\n",i++,
			//coredev,((struct smscore_idlist_t *)nextid)->data_type, ((struct smscore_idlist_t *) nextid)->id,clienttest->context);
 			if ( ((struct smscore_idlist_t *)nextid)->data_type == data_type) 
			{
				client = (struct smscore_client_t *) next;
				break;
			}
...
int  smscore_wakeup_spi(struct smscore_device_t *coredev, int data_type)
{
	struct smscore_client_t *pclient = NULL;

	//printk(KERN_EMERG"[zm2167] smscore_wakeup_spi1  coredev=%d\n",coredev);
	if( coredev != NULL )
	{
		pclient = smscore_find_client2(coredev,data_type,0);
		if( pclient != NULL )
		{
			//printk(KERN_EMERG"[zm2167] smscore_wakeup_spi2  pclient=%d  dev=%d\n",pclient,pclient->context);
			return smschar_wakeup_spi(pclient->context);
		}
	}
...
{
    smsspi_poweron();
    smscore_load_firmware_family2(panic_core_dev,(void*)siano_fwimage_2186,siano_fwsize_2186);
    mdelay(100);
