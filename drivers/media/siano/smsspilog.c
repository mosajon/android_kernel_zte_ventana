...
#ifdef ZTE_CMMB_ZM2167
int smschar_parseCount = 0;
extern int smschar_parseMsgl(char * buffer);
extern int smscore_wakeup_spi(struct smscore_device_t *coredev,int data_type);
int  sms_wake_up_spi(int data_type) //0: ctl   607:data
{
	//_spi_dev.context->	_spi_device_st.coredev ->  smscore_device_t.smscore_find_client() -> smscore_client_t.context -> smschar_device_t
	//struct _spi_device_st   *pspi_device_s = spidev->context;
	//struct smscore_device_t *pcore_dev = spi_dev->coredev;
	Zi_log("data_type=%d  spi_dev=%d\n",data_type,spi_dev);
	if( spi_dev!=NULL)
		return smscore_wakeup_spi(spi_dev->coredev,data_type);
...
#ifdef ZTE_CMMB_ZM2167
/*shangmingsen 20120417 */
void zi_DataSpiTxRx(unsigned char *txbuf,
		     unsigned char *rxbuf, int len)
{
    struct _spi_device_st *spi_device = spi_dev;
    smsspibus_xfer((spi_device->dev).phy_context, txbuf, 0, rxbuf, 0, len);
}
#endif
		...
		if (!msg && !list_empty(&spi_device->txqueue))
			...
			if (msg->add_preamble) {
				...
				memset(&txmsg.buf[txmsg.len], 0,
					...
					if( CMMB_ZM2167_DEVICE == cmmb_chip_type )
					{
						while(Zi_irq_count > 0)
						{
							if(Zi_pCmmbDevice)
								Zi_CmdProcess(Zi_pCmmbDevice);
							Zi_irq_count--;
							g_Sms_MsgFound_Counter++;
                                     
                                }
						smschar_parseMsgl(txbuffer);	
                                }
					else
				        #endif
					{							
...
#ifdef ZTE_CMMB_ZM2167
		if( cmmb_chip_type == CMMB_ZM2167_DEVICE)
		{
			msg.alignment = 272;
			msg.add_preamble = 0;
		}
