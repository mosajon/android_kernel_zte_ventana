...
struct delayed_work taos_als_work;
...
#if (defined(CONFIG_PM) && defined(CONFIG_TMD2771_SUSPEND_POWEROFF))
unsigned int once_at_call = 0;
...
#if I2C_LEGACY_PM
static int tmd2771x_legacy_suspend(struct i2c_client *clientp, pm_message_t mesg)
{
...
static int tmd2771x_legacy_resume(struct i2c_client *clientp)
{
#ifdef CONFIG_TMD2771_SUSPEND_POWEROFF
	struct tmd2771x_platform_data *pdata;
	...
	if(g_als_config.enabled && !g_prox_config.enabled){
		enable_reg |= TMD2771X_ENBL_ALS_INT;
	}
...
static struct i2c_driver tmd2771x_driver = {
	...
	.suspend = tmd2771x_legacy_suspend,
	.resume	= tmd2771x_legacy_resume,
#else
	.driver = {
		.name	= "tmd2771x",
		.owner	= THIS_MODULE,
