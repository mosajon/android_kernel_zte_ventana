...
static struct sensors_item_info sensors_info_list[] = {
	...
	SENSORS_INFO(0x1D, ID_EXIST, 0x0D, 0x2A, 0, SENSORS_TYPE_ACCEL, SENSORS_VENDOR_FREESCALE),
	...
	SENSORS_INFO(0x1D, ID_EXIST, 0x0D, 0x2A, 2, SENSORS_TYPE_ACCEL, SENSORS_VENDOR_FREESCALE),
	SENSORS_INFO(0x19, ID_EXIST, 0x0F, 0x33, 2, SENSORS_TYPE_ACCEL, SENSORS_VENDOR_ST),
	SENSORS_INFO(0x0F, ID_EXIST, 0x0F, 0x05, 2, SENSORS_TYPE_ACCEL, SENSORS_VENDOR_KIONIX),
	SENSORS_INFO(0x0E, ID_EXIST, 0x00, 0x48, 2, SENSORS_TYPE_COMPASS, SENSORS_VENDOR_AKM),
	SENSORS_INFO(0x68, ID_NOEXIST, 0x00, 0x00, 2, SENSORS_TYPE_GYRO, SENSORS_VENDOR_INVENSENSE),
#endif
            ...
            case SENSORS_CHECK_GET_COMPASS:
                for(i = 0; i < sensors_num; i++){
                    if(SENSORS_TYPE_COMPASS== sensors_info_list[i].type){
                        //ret = sensor_check(&sensors_info_list[i]);
                        ret=AkmTestProcess_Body(); //ZTE: added by liumx temp for compass autotest
                        if(!ret){
                            sensor_sate = 1;
                        }
                    }
