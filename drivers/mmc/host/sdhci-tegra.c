...
static irqreturn_t carddetect_irq(int irq, void *data)
{
	...
	plat = pdev->dev.platform_data;
	if (plat->mmc_data.status) {
		card_present = plat->mmc_data.status(mmc_dev(sdhost->mmc));
		printk("%s: card is %s\n", mmc_hostname(sdhost->mmc), card_present?"plugged":"removed");
	}
				  ...
				  struct sdhci_pltfm_data *pdata)
{
	...
	if (plat->mmc_data.status) {
		...
		printk("%s: card is %s", mmc_hostname(host->mmc), \
                        plat->mmc_data.card_present?"present":"not present");
	}
