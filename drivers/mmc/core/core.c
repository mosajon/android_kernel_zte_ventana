 ...
 *	response.
 */
static DEFINE_MUTEX(mutex_data_ops);
            ...
            if ((mrq->cmd->error == -EILSEQ)
                ...
                */
                if (host->ios.clock <= 3000000) {
                    printk(KERN_WARNING "%s:SD clock is too slow!\n", mmc_hostname(host));
                    break;
                }

                printk(KERN_WARNING "%s: request error! slow the clock to %d\n",
                mmc_hostname(host), host->ios.clock/2);
                mmc_set_clock(host, host->ios.clock/2);
                continue;
            }else if (mrq->data && (mrq->data->error == -ETIMEDOUT)){
                if(host->ios.clock > 26000000){
                    printk(KERN_WARNING "%s: request timeout! slow the clock to 26M\n",
                                                        mmc_hostname(host));
                    mmc_set_clock(host, 26000000);
                    continue;
                }
            }
                break;
