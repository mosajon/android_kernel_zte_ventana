...
static void tegra_dsi_hs_clk_out_enable_in_lp(struct tegra_dc_dsi_data *dsi)
{
	...
	tegra_dsi_hs_clk_out_enable(dsi);
       printk("tegra_dsi_hs_clk_out_enable_in_lp\n");
...
/*Nvidia Danny add for error waveform 20120203*/
#if 0
       if(initflag == 0)
       {
	if (!dsi->ulpm) {
       ...
       #endif
/*Nvidia Danny add for error waveform 20120203 end */
						...
						struct tegra_dc_dsi_data *dsi)
{
	...
	tegra_dsi_pad_calibration(dsi);
        /*nvidia danny modify for error waveform 2012.02.03*/
...
/*nvidia danny modify for error waveform 2012.02.03 end */
...
extern int ventana_panel_set_lcd_type_for_hx8369(int panelid);
extern int ventana_panel_set_lcd_type(int panelid);
					...
					u32 n_cmd)
{
    ...
    tegra_dsi_read_data(dc, dsi, 10, 0xA, buffer);
    printk("[tegra_dsi_send_panel_cmd ]tegra_dsi_read_data A %x %x, %x n_cmd %d", buffer[0], buffer[1], buffer[2], n_cmd);
#endif
    err = 0;
    for (i = 0; i < n_cmd; i++) {
				...
				u32 panel_reg_addr, u8 *read_data)
{
	...
	tegra_dsi_reset_read_count(dsi);
       printk("tegra_dsi_read_data dsi->status.lp_op %d\n", dsi->status.lp_op);
...
{
#if 1  /*nvidia danny delete for error waveform 2012.03.03*/
	u32 val;
	int ret;
...
static int tegra_dsi_exit_ulpm(struct tegra_dc_dsi_data *dsi)
{
	...
	dsi->ulpm = false;
 /*nvidia danny delete for error waveform 2012.03.03*/
...
#ifdef CONFIG_MACH_VENTANA
static int  tegra_dc_dsi_set_lcd_type(struct tegra_dc *dc)
{
	struct tegra_dc_dsi_data *dsi = tegra_dc_get_outdata(dc);
        int panel_id, i, err;
        u8 buffer[10];
  	struct tegra_dsi_cmd *p_cmd;
        ...
        printk("[tegra_dc_dsi_set_lcd_type ] before n_init_cmd %x", dsi_pdata->n_init_cmd);
#if CONFIG_LCD_480_800
        printk("[tegra_dc_dsi_set_lcd_type ]tegra_dsi_read_data 0xDA %x %x, %x", buffer[0], buffer[5], buffer[2]);
        panel_id = buffer[5];
        printk("[tegra_dc_dsi_set_lcd_type ] before n_init_cmd %x", dsi_pdata->n_init_cmd);
        ventana_panel_set_lcd_type_for_hx8369(panel_id);          
#else
        printk("[tegra_dc_dsi_set_lcd_type ]tegra_dsi_read_data 0xDA %x %x, %x", buffer[0], buffer[1], buffer[2]);
        panel_id = buffer[1];
        printk("[tegra_dc_dsi_set_lcd_type ] before n_init_cmd %x", dsi_pdata->n_init_cmd);

           ventana_panel_set_lcd_type(panel_id);
#endif
        
        printk("[tegra_dc_dsi_set_lcd_type ] end n_init_cmd %x", dsi_pdata->n_init_cmd);

	/* Copy panel init cmd */
       if(dsi->info.dsi_init_cmd)
	{
		/* free up the pdata */
            ...
            dsi->info.dsi_init_cmd = NULL;
        }
       dsi->info.dsi_init_cmd = kzalloc(sizeof(*p_cmd) *
	...
	if (!dsi->info.dsi_init_cmd) {
      ...
      dsi->info.n_init_cmd = dsi_pdata->n_init_cmd;
	err = tegra_dc_dsi_cp_p_cmd(dsi_pdata->dsi_init_cmd,
						dsi->info.dsi_init_cmd, dsi_pdata->n_init_cmd);
	if (err < 0)
		return err;
        if(dsi->info.dsi_late_resume_cmd)
	 {
		/* free up the pdata */
            for (i = 0; i < dsi->info.n_late_resume_cmd; i++) {
            	if (dsi->info.dsi_late_resume_cmd[i].pdata)
            		kfree(dsi->info.dsi_late_resume_cmd[i].pdata);
            }
            kfree(dsi->info.dsi_late_resume_cmd);
            dsi->info.dsi_late_resume_cmd = NULL;
        }
       dsi->info.n_late_resume_cmd = dsi_pdata->n_late_resume_cmd;       
       dsi->info.dsi_late_resume_cmd = kzalloc(sizeof(*p_cmd) *
					dsi_pdata->n_late_resume_cmd,
					GFP_KERNEL);
	if (!dsi->info.dsi_late_resume_cmd) {
		err = -ENOMEM;
		return err;
	}
	err = tegra_dc_dsi_cp_p_cmd(dsi_pdata->dsi_late_resume_cmd,
...
static void tegra_dc_dsi_disable(struct tegra_dc *dc)
{
	...
	if (dsi->status.dc_stream == DSI_DC_STREAM_ENABLE)
		tegra_dsi_stop_dc_stream_at_frame_end(dc, dsi);
        mdelay(50); //nvidia martin add at 2011.12.17
