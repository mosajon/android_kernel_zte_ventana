...
static int tegra_dc_set_dynamic_emc(struct tegra_dc_win *windows[], int n)
{
	...
	if ( new_rate > dc->new_emc_clk_rate )
...
static bool _tegra_dc_controller_enable(struct tegra_dc *dc)
{
	if (dc->out->enable)
		dc->out->enable();
       printk("_tegra_dc_controller_enable out enable IRQ %d\n", dc->irq);
...
/*	if (dc->ndev->id == 0 && tegra_dcs[1] != NULL) {
    ...
    */
	...
	enable_dc_irq(dc->irq);
       printk("_tegra_dc_controller_enable enable IRQ\n");
...
/*zte 10094834 add for black screen after recovery mode restart at 20120619*/
#ifdef CONFIG_MACH_VENTANA
...
static int tegra_dc_shutdown(struct nvhost_device *ndev)
{
	struct tegra_dc *dc = nvhost_get_drvdata(ndev);
       ...
       if(s_SuspendFlag==1 )
      {
            printk( "dc shutdown has suspend %d \n", s_SuspendFlag);
            return 0;
       }
...
#endif
/*zte 10094834 add for black screen after recovery mode restart  at 20120619 end*/
