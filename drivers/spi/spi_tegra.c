...
static int spi_tegra_alloc_dma(struct spi_device *spi)
{
	struct spi_tegra_data *tspi = spi_master_get_devdata(spi->master);
	int ret = 0;
	pr_info("%s\n", __func__);
	if(tspi->is_dynamic_dma){
		pr_info("%s 2\n", __func__);	
		tspi->rx_dma = tegra_dma_allocate_channel(TEGRA_DMA_MODE_ONESHOT,
				"spi_rx_%d", tspi->pdev->id);
		if (!tspi->rx_dma) {
			dev_err(&spi->dev, "can not allocate rx dma channel\n");
			ret = -ENODEV;
			goto exit;
		}

		tspi->tx_dma = tegra_dma_allocate_channel(TEGRA_DMA_MODE_ONESHOT,
					"spi_tx_%d", tspi->pdev->id);
		if (!tspi->tx_dma) {
			dev_err(&spi->dev, "can not allocate tx dma channel\n");
			ret = -ENODEV;
			if (tspi->rx_dma)
...
static int spi_tegra_free_dma(struct spi_device *spi)
{
	struct spi_tegra_data *tspi = spi_master_get_devdata(spi->master);
	pr_info("%s\n", __func__);
	if(tspi->is_dynamic_dma){
...
static int __init spi_tegra_probe(struct platform_device *pdev)
{
	...
	master->alloc_dma = spi_tegra_alloc_dma;
	master->free_dma = spi_tegra_free_dma;
	...
	if (pdata) {
		...
		tspi->is_dynamic_dma = pdata->is_dynamic_dma;
	} else {
